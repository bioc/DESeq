...
approach that is based on error modeling and the concept of
variance stabilizing transformations~\cite{Tibshirani1988,sagmb2003,Anders:2010:GB}. We first estimate an
overall mean-dispersion relationship of the data using
\Rfunction{estimateDispersions} with the argument
\Robject{method="blind"} and then call the function
...
@
%
The resulting transformation is shown in the left panel of Figure~\ref{figvsd} (the code
that produces the figure is hidden from this vignette for the sake of
presentation, but can be seen in the \texttt{.Rnw} or \texttt{.R}
source files).
%
\fixme{\Rfunction{getVarianceStabilizedData} should do the offset
  correction by a common offset \Robject{mean(offsets)} computed like
  in the following code:}
<<offsets>>=
scaledCounts = counts(cds, normalized=TRUE)
offsets = sapply(seq_len(ncol(vsd)), function(j) {
    cj = scaledCounts[, j]
    wh = which.max(cj)
    log2(cj[wh]) - vsd[wh, j]
})
...
offset = mean(offsets)
@
<<vsd, fig=TRUE, echo=FALSE, width=5, height=5>>=
...
colors = c("blue", "black")
matplot(px[ord],
        cbind(vsd[, 1] + offset, log2(px))[ord, ],
        type="l", lty=1, col=colors, xlab="n", ylab="y")
legend("bottomright",
       legend = c(
...
For plotting (right panel of Figure~\ref{figvsd}), we replace the infinite values by an arbitrary fixed large number:
<<repl>>=
largeNumber <- 7
...
\begin{figure}
\centering
\includegraphics[width=.49\textwidth]{DESeq-vsd}
\includegraphics[width=.49\textwidth]{DESeq-figmodlr}
\caption{Left: Graphs of the variance stabilizing transformation for sample 1
  (blue) and of the transformation $y=log_2(n)$ (black).
  Right: scatterplot of direct (\Robject{lfc}) versus \emph{moderated} log-ratios (\Robject{mod\_lfc}).
