...
S.\ Anders, W.\ Huber: ``Differential expression analysis for sequence count data''. 
This paper is currently under review. A preprint can be obtained from Nature Preceedings: 
\url{http://dx.doi.org/10.1038/npre.2010.4282.1}}
\end{abstract}

\section{Quick start}

This first section just shows the commands necessary for an analysis at a glance. For a more gentle introduction, start reading at Section \ref{sec:prep}.
...
For the condition ``Tb'', we cannot estimate a variance function  as we have no replicates. When a variance estimate is needed for ``Tb'', the package will use the
...
rawVarFuncTable( cds )
@
The first column shows the assignment. The second column (variance adjustment factors)
is explained below.
...
the green line) is usually nothing to worry about; just leaving it as it is might be the prudent
thing to do as it cause DESeq to be conservative with the tests.

Even though this should normally not be necessary, DESeq offers the possibility to
set variance adjustment factors to adjust the variance estimates. By default, these factors
are all 1:
<<>>=
varAdjFactors( cds )
@
...
\begin{figure}
\centering
\includegraphics[width=.5\textwidth]{DESeq-figVarRed}
\caption{Effect of setting the variance adjustmnet factor for condition ``T'' to 2.3.}
\label{figVarRed}
\end{figure}

If you want to inflate the variance estimates to cause DESeq to become more conservative, 
set them to a value above 1.
<<>>=
varAdjFactors( cds ) <- c( N=2.3, T=1, Tb=1 )
@

Now, the variance estimates are increased and the ECDF lines are shifted upwards 
(Fig. \ref{figVarRed}).
<<figVarRed,fig=TRUE,width=7,height=7>>=
residualsEcdfPlot( cds, "N" )
@ % $

Let's switch back to the default value before continuing:
<<>>=
varAdjFactors( cds ) <- c( N=1, T=1, Tb=1 )
@

\section{Calling differential expression} \label{sec:DE}
...
Finally, we may filter for the significant genes,
<<>>=
...
@
As can be seen, we have still found about 1/4 of the hits, and only a reassuringly small number of 
...
pvals2b <- nbinomTestForMatrices(
   ...
   rawVarFunc( cds2, "_pooled", TRUE )( baseMeansNT ),
   rawVarFunc( cds2, "_pooled", TRUE )( baseMeansNT ) )
@
...
Another feature of the \Rpackage{DESeq} package is the variance stabilising transformation (VST).
...
of the mean, i.e., it produces a homoscedastic version of the data.
...
variance-stabilising transformation of the count data.}
\label{figHeatmap}
\end{figure}

This data is now approximately homoscedastic and hence suitable as input to various statistical procedures, e.g., a simple distance calculation:
<<>>=
dists <- dist( t( vsd ) )
@

We visualise the distance matrix as a heatmap:
<<figHeatmap,fig=TRUE>>=
...
Finally, the slot \texttt{rawVarFuncTable} contains a data frame whose \texttt{fundName} column serves as a look-up table. Its other column, \texttt{varAdjFactor} stores the adjustemtn factor which which the
variance estimate from the \texttt{rawVarFuncs} should be scaled.
