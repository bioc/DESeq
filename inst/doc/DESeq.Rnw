...
We check the fit (Fig.~\ref{figFitTwithQuantilesFull}):
...
\begin{figure}
\centering
\includegraphics[width=.6\textwidth]{DESeq-figHeatmap1}
...
<<figHeatmap1,fig=TRUE,width=7,height=7>>=
heatmap(as.matrix( dists ), 
        symm=TRUE,
        scale="none", margins=10*c(1,1),
        col = colorRampPalette(c("darkblue","white"))(100))
@

We can also visualise the expression data of, say, the top 40 differentially expressed genes.
%
<<figHeatmap2a,fig=TRUE,width=7,height=10>>=
select = order(res$pval)[seq_len(40)]
