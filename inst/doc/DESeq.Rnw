...
DESeq orginally (as described in the DESeq paper \cite{Anders:2010:GB}) only
used the fitted values (\texttt{sharingMode="fit-only"}). The
current default (\texttt{sharingMode="maximum"}) is more conservative.

Another difference of the current DESeq version to the original method described in the
...
<<table>>=
table( "all-samples-simple" = resFullB$padj < 0.1,
...
Since our paper on DESeq was published in Genome Biology in Oct 2010, we made a
number of changes to algorithm and implementation, which are listed here.
...
Equation~(11) of the paper defines ``more extreme'' as those values pairs $(a,b)$
that have smaller probability than the observed one. This caused problems in case
that the dispersion exceeds 1. Hence, we now sum instead the probabilities of all
values pairs that are ``further out'' in the sense that they cause a more extreme
fold change $(a/s_A)/(b/s_B)$, where $s_A$ are the sums of the size factors of
the samples in conditions $A$ and $B$. We do this in a one-tailed sense and 
double the result. Furthermore, we now longer approximate the sum but always 
calculate it exactly.

\item We added the possibility to fit GLMs of the negative binomial family with
log link as described in this vignette. $p$ values are calculated by a 
standard $\chi^2$ likelihood ratio test. The logarithms of the size factors are 
are given as offsets to the GLM fitting function.
...
\Rfunction{estimateDispersion} allows the user to go back to the old method.
...
\emph{fitType} to \emph{estimateDispersion} allows to choose between these.

\item Finally, we replaced the term \emph{raw squared coefficient of variance}
used in the paper with the more standard term \emph{dispersion}.
