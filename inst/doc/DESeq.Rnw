...
\fixme{\begin{enumerate}
  ...
  \item The boundary effects of the smoothing line look awful - I understand this may not
...
plotFitInfo = function(cds, conds = ls(cds@fitInfo), probs = 0.99) {
  ...
  xg = 10^seq( log10(min(means[means>0])), 
               log10(max(means)), length.out=100 )
  for(cond in conds){
