...
noise, is the dominating noise source for lowly expressed genes and is not
...
\begin{figure}
\centering
\includegraphics[width=.66\textwidth]{DESeq-figFit}
...
par( mfrow=c(1,2) )
plot(   means, cds@fitInfo[["untreated"]]$perGeneDispEsts, pch=".", log="xy" )
points( means, cds@fitInfo[["untreated"]]$fittedDispEsts,  pch=".", col="red" )
plot(   means, cds@fitInfo[[ "treated" ]]$perGeneDispEsts, pch=".", log="xy" )
points( means, cds@fitInfo[[ "treated" ]]$fittedDispEsts,  pch=".", col="red" )
@

The plot (Fig.~\ref{figFit}) is doubly logarithmic; this may be helpful or
...
Can we really assume these to have the same variance as the other genes
...
plot with lines indicating the 5- and 95-percentil of this distribution (Fig.\ 
\ref{figFitTwQ}):
...
plot( ( ncu[,1]+ncu[,2] )/2, log2( ncu[,2] / ncu[,1] ), pch=".", log="x" )
@

As one can see (Fig.\ \ref{figMArepl}), the log fold changes between replicates 
are stronger for lowly expressed genes than for highly expressed ones. We 
may conclude that a gene's expression is influenced by the treatment only
if the change between treated and untreated samples are strong as compared to
what we see between replicates, and hence, the dividing line between red and black in
Fig.\ \ref{figDE} mimics the shape seen in Fig.\ \ref{figMArepl}.\footnote{}
