...
As an example dataset, we use the
gene level read counts from the \Rpackage{pasilla} package,
which presents data by \citet{Brooks2010}. This data set is from a study using
\emph{Drosophila melanogaster} cell cultures and studying the effect of a 
knock-down of the gene \emph{pasilla}, a spliciong factor.

In the \Rpackage{pasilla} package, the data is presented as the object called \Robject{pasillaGenes},
of class \Rclass{CountDataSet}, which is the data structure used internally by 
\Rpackage{DESeq}. We load the \Rpackage{DESeq} and then the pasilla data as follows.
...
\Robject{pasillaGenes} contains the counts and also meta information about the samples:
...
As you can see, the samples differ in their condition (untreated or treated, i.e., with
pasilla knocked down) and in the library type. To keep things simple, we will only 
...
%  arguments of \Rfunction{nbinomTest}.} --> Doesn't matter, one should look at both.
