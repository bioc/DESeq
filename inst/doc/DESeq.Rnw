...
\section{Quick start}
%--------------------------------------------------
This first section quickly presents a complete analysis workflow in one go. 
A more gentle introduction is provided Section~\ref{sec:prep} and subsequent.

We load the \Rpackage{DESeq} package and an example dataset, the
gene level read counts from the \Robject{pasilla} dataset.
...
%
<<makecds>>=
cds = pasillaGenes[, pasillaGenes$type=="paired-end"]
cds@multivariateConditions = FALSE   ## ugly
...
Each row corresponds to the entities for which you compare coverage, 
e.\,g.\ to a gene, to a binding region in a ChIP-Seq dataset, 
a window in CNV-Seq, or the like.

Each column must stem from an independent biological replicate.  For
purely technical replicates (e.\,g. when the same library preparation was
distributed over multiple lanes of the sequencer in order to increase
coverage), please sum up their counts to get a single column,
corresponding to a unique biological replicate.  This is important in
order to allow \Rpackage{DESEq} to estimate variability in the
experiment correctly.

The data thus looks as follows
<<countsTable,eval=FALSE,echo=FALSE>>=
head(counts(pasillaGenes))
pData(pasillaGenes)
@
