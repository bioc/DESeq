...
intermediate steps, a \Robject{fitInfo} object is stored, which contains
...
plot the per-gene estimates against the normalized mean expressions
per gene and overlay the fitted curve by using the helper function
