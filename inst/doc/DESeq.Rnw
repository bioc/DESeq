...
\begin{abstract}
A basic task in the analysis of count data from RNA-Seq and analogous assay types 
(e.\,g.\ comparative ChIP-Seq) is the detection of differentially expressed genes.
The count data are presented as a table which reports, for each sample, the number of
...
%--------------------------------------------------
This first section quickly gives you an overview over a complete
analysis workflow in one go.  A more gentle introduction is provided
Section~\ref{sec:prep} and subsequent.

We load the \Rpackage{DESeq} package and an example dataset, the
gene level read counts from the \Rpackage{pasilla} package,
which presents a dataset by \citet{Brooks2010}.
...
<<countsTable,eval=FALSE,echo=FALSE>>=
head(counts(pasillaGenes))
pData(pasillaGenes)
@

<<condNames,echo=FALSE>>=
condNames = unique(pasillaGenes$condition)
stopifnot(setequal(c("treated", "untreated"), condNames))
@ 

We want to perform a simple pairwise comparison between the two
conditions \Sexpr{condNames[1]} and \Sexpr{condNames[2]}. To side-step
batch effects, we look only at the paired-end data\footnote{We will
  see how to do more complex, factorial analyses that can absorb batch
  effects in Section~\ref{sec:fixme}.}.
Then, the minimal set of commands to run a full analysis is:
%
<<makepgp>>=
pgp = pasillaGenes[, pasillaGenes$type=="paired-end"]
pgp@multivariateConditions = FALSE   ## ugly
pgp$condition = factor(pgp$condition)
@ 
<<demo1, cache=TRUE>>=
pgp = estimateSizeFactors( pgp )
pgp = estimateDispersions( pgp )
res = nbinomTest( pgp, "untreated", "treated")
@

\Rfunction{nbinomTest} tests for differential expression between the
two conditions and returns a data frame with $p$ values (raw and
adjusted), mean values, fold changes, and other useful information,
...
a window in CNV-Seq or the like.

It is important that each column stems from an independent biological
...
replicate.  This is important in order to allow \Rpackage{DESEq} to
estimate variability in the experiment correctly.

Let us start with a table of counts:
%
<<countsTable>>=
countsTable = counts(pgp)
head(countsTable)
@ 
%
Here, we have generated this matrix by extracting it from the object
\Robject{pgp} that we created above. For your own data, you will need
to create from the sequence alignments by use of tools outside of the
...
packages\footnote{Another, easy way to produce such a table from the output
...
As a second component of the analysis, you need a \Rclass{factor}
variable with as many elements as there are columns in
\texttt{countTable}. It indicates treatment groups. Here, we use
the column \Robject{condition} of the \Robject{pgp} object.
<<condition>>=
conds = factor(pgp$condition)
conds
@ 
%
For your own data, you will need to create an analogous
\Rclass{factor} variable from the metadata of your dataset.
...
slots\footnote{In fact, the objects \Robject{pasillaGenes} and \Robject{cds}
  from Section~\ref{sec:quickstart} are also of class \Rclass{CountDataSet};
...
the sample mean and variance within replicates \fixme{this needs to be updated, perhaps
  not mant details here, but then please provide them later} and fitting a curve to this graph.
...
@
%
\fixme{Should \Robject{cond} be \Robject{treated}? Need to check order / asymmetry between the 
  arguments of \Rfunction{nbinomTest}.}
...
%column.
%<<>>=
%diagForT <- varianceFitDiagnostics( cds, "treated" )
%head( diagForT )
%@
...
%par( mfrow=c(1,2 ) )
%residualsEcdfPlot( cds, "treated" )
%residualsEcdfPlot( cds, "untreated" )
%@
...
<<>>=
resTbvsN <- nbinomTest( cds, "untreated", "treated" )
@
...
%<<>>=
%colsN <- conditions(cds) == "untreated"
%colsT <- conditions(cds) == "treated"
