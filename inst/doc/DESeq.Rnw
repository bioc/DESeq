...
expressions per gene, and then overlay the fitted curve in red. The following 
function does this.
...
%<<plotFitInfo>>=
%plotFitInfo <- function(cds, cond = NULL, probs = c( .01, .05, .95, .99) ) {
%  means <- rowMeans( counts( cds, normalized=TRUE ) )
%  xg <- 10^seq( log10(min(means[means>0])), log10(max(means)), length.out=300 )
%  df <- cds@fitInfo[[cond]]$df
%  qvals <- cds@fitInfo[[cond]]$dispFunc( xg ) %*% rbind(c(1, qchisq( probs, df )/df))
%  pge <- cds@fitInfo[[cond]][["perGeneDispEsts"]]
%  matplot(xg,
%    qvals,
%    col = c( "red", rep( "blue", length(probs) ) ), 
%    lty = 1, type = "l", log = "xy", ylim = range( c( qvals, pge ), na.rm=TRUE ),
%    main = cond, ylab = "dispersions", xlab= "average normalized count" )
%  points(means, pge, pch=".")
%}
%@   
...
We call it for both conditions (Fig.\ \ref{figFit})

<<figFit,fig=TRUE,width=11,height=5.5>>=
par( mfrow=c(1,2) )
plotDispEsts( cds, "untreated" )
plotDispEsts( cds, "treated" )
@
...
\fixme{Some of the function calls below produce warnings: \texttt{glm.fit: algorithm did not converge}.
Either modify code so that they do not occur, or explain to the users when they should worry about this and when not.}
...
\fixme{Explain the bins with exceptionally large counts, in particular those at 1 and $\sim$0.45.}
%
<<histp,fig=TRUE>>=
...
cdsFullB <- estimateDispersions( cdsFullB, method="pooled", 
                                 sharingMode="maximum" )
...
\caption{Heatmaps showing the expression data of the top \Sexpr{length(select)}
  differentially expressed genes. Left: clustering of the variance stabilisation 
  transformed data (\Robject{vsd}),
  right: clustering of the original count data (\Robject{counts{cds}}).
