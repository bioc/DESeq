...
this vignette for the sake of presentation, but can be seen in the
\texttt{.Rnw} or \texttt{.R} source files.
%
\begin{figure}
\centering
...
\caption{Left: Graphs of the variance stabilizing transformation for
  ...
  black. $s_1$ is the size factor for the first sample.
  Right: density plot of the per-gene standard deviation of the transformed data,
  across samples, against the rank of the mean. \fixme{The trend in
  this plot should be flat -- something is wrong.}
}
\label{figvsd}
\end{figure}
