...
If in your data the residuals ECDF plot indicates problems with the fit, you should
manually adjsut the variance estimates. If the ECDF curves are above the green line, variance is 
...
If the ECDF curves are below the green line, variance is overestimated, which leads to 
too high p values (and hence, an overestimation of the false discovery rate). 

You may set variance adjustment factors to adjust the estimates. By default, these factors
are all 1:
<<>>=
varAdjFactors( cds )
@
...
\caption{Effect of setting the variance adjustmnet factor for condition ``T'' to 0.4.}
\label{figVarRed}
\end{figure}

If you want to reduce the variance factors, set them to a value below 1.
<<>>=
varAdjFactors( cds ) <- c( N=.4, T=1, Tb=1 )
@

Now, the variance estimates are reduced and the ECDF lines are shifted downwards 
(Fig. \ref{figVarRed}).
<<figVarRed,fig=TRUE,width=7,height=7>>=
residualsEcdfPlot( cds, "N" )
@ % $

Let's switch back to the default value before continueing:
<<>>=
