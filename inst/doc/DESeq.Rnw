...
However, one seems to get better results by estimating the size factors from the count data using
robust linear regression (with the \Rfunction{rlm} function from the \Rpackage{MASS} package). The
function \Rfunction{estimateSizeFactors} does that for you.
<<>>=
cds <- estimateSizeFactors( cds )
sizeFactors( cds )
@
