...
% library('cacheSweave'); rm(list=ls()); Sweave('DESeq.Rnw', driver=cacheSweaveDriver())
...
\footnote{If you use htseq-count, be sure to remove the extra lines with general
counters (``ambiguous'' etc.) at the end of the count table when importing the data.}
...
statistical validity relies on property \ref{it:indp} --which is
...
imply from first principles, but rarely a problem in practice.  We refer
to~\cite{Bourgon:2010:PNAS} for further discussion of this topic.
<<rs>>=
...
of chosen the parameter equivalent to $n_0$ above. It is based on
